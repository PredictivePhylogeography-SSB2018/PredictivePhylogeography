...
The Mean Decrease in Accuracy (MDA) is a measure of variable importance that is calculated based on permuting the data and omitting a predictor variable. MDA is the increase in percent of times a case is OOB and misclassified when the variable is permuted. Gini impurity (GINI), on the other hand, measures variable importance based on how variables contribute to node purity In other words, if, when used, a variable results in splits that generally split between, not within, classes, then that variable increases node purity. Variables that increase node purity will have higher mean decreases in GINI.
...
Since the donwsampling scheme using LC and NoLC as the response variable gives us the most accurate results, we will use this model. We will average the prediction across all iterations. The `type="prob"` command will give a probablily for each species belonging to each category, given this classifier.
```{r}
